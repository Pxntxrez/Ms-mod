...
public class SwarmWorldDiggerBase {
    ...
    public boolean bridge(int x, int y, int z) {
        ...
        // FIX (configurable): don't place on torches, slabs, or anything that isn't a full
        // solid surface. Disabled when "Place On Partial Blocks" is true (original behavior).
        if (!Config.PLACE_ON_PARTIAL
                && !this.sw.world.isSideSolid(new BlockPos(x, y - 1, z), EnumFacing.UP))
            return false;

        IBlockState state = this.sw.world.getBlockState(pos);
        Block type = state.getBlock();

        // FIX (configurable): don't pave over liquids. Liquids report isReplaceable()==true,
        // so without this they get filled in wherever there is solid ground beneath them.
        // Water/lava have their own toggles; other (modded) liquids need both enabled.
        Material mat = state.getMaterial();
        if (mat == Material.WATER) {
            if (!Config.PLACE_IN_WATER)
                return false;
        } else if (mat == Material.LAVA) {
            if (!Config.PLACE_IN_LAVA)
                return false;
        } else if (mat.isLiquid()) {
            if (!(Config.PLACE_IN_WATER && Config.PLACE_IN_LAVA))
                return false;
        }

        // FIX (configurable): only replace truly replaceable blocks (air, grass, flowers...).
        // Disabled when "Replace Non-Full Blocks" is true (original behavior), allowing
        // torches, slabs, etc. to be replaced.
        if (!Config.PLACE_REPLACE_NONFULL
                && type != null && !type.isReplaceable(this.sw.world, pos))
            return false;
